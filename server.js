...
const REG_CODE_TTL_MS = 30 * 60 * 1000; // 30 минут
...
<body>
  ...
  <div class="info-card" style="max-width:820px;width:100%;">
    ...
    <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
      <button class="ep" style="cursor:pointer;border:none;" onclick="createRegCode()">✚ Создать новый код (30 мин)</button>
      <button class="ep" style="cursor:pointer;border:none;" onclick="deactivateRegCode()">⛔ Деактивировать</button>
    </div>
    <div style="font-size:10px;color:var(--text3);margin-top:8px;font-weight:600;">Код действует 30 минут с момента создания, после чего перестаёт работать сам по себе. Его можно деактивировать раньше вручную. Код не привязан к аккаунтам — если его деактивировать или он истечёт, уже созданные через него аккаунты не удаляются и продолжают работать как обычно. Требует ключ администратора (ADMIN_KEY).</div>
  </div>
  <div class="info-card" style="max-width:820px;width:100%;">
  ...
  <script>
    ...
    async function refreshRegCodeStatus() {
      try {
        const r = await fetch('/admin/regcode/status');
        const d = await r.json();
        renderRegCodeStatus(d.active, d.expiresAt);
      } catch(e) { document.getElementById('regcode-status').textContent = 'Ошибка загрузки статуса'; }
    }
    async function createRegCode() {
      const adminKey = prompt('Ключ администратора (ADMIN_KEY):');
      if (!adminKey) return;
      try {
        const r = await fetch('/admin/regcode/create', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey }) });
        const d = await r.json();
        if (!d.ok) { alert('Ошибка: ' + (d.error||'')); return; }
        _lastCreatedRegCode = d.code;
        renderRegCodeStatus(true, d.expiresAt);
        prompt('Новый код регистрации (действует 30 минут, скопируйте):', d.code);
    ...
    async function deactivateRegCode() {
      ...
      if (!adminKey) return;
      try {
        const r = await fetch('/admin/regcode/deactivate', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey }) });
        const d = await r.json();
        if (!d.ok) { alert('Ошибка: ' + (d.error||'')); return; }
        refreshRegCodeStatus();
        alert('Код деактивирован. Уже созданные через него аккаунты остались.');
      } catch(e) { alert('Сервер недоступен'); }
    }
    refreshRegCodeStatus();
    ...
    async function toggleUserDisabled(login, disabled) {
      ...
      if (!adminKey) return;
      try {
        const r = await fetch('/admin/users/toggle-disabled', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey, login, disabled }) });
        const d = await r.json();
        if (!d.ok) { alert('Ошибка: ' + (d.error||'')); return; }
    ...
    async function resetUserPassword(login) {
      const adminKey = prompt('Ключ администратора (ADMIN_KEY):');
      if (!adminKey) return;
      if (!confirm('Выдать пользователю ' + login + ' новый пароль? Старый перестанет работать.')) return;
      try {
        const r = await fetch('/admin/users/reset-password', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey, login }) });
        const d = await r.json();
        if (!d.ok) { alert('Ошибка: ' + (d.error||'')); return; }
        prompt('Новый пароль для ' + login + ' (передайте его пользователю, он больше нигде не будет показан):', d.newPassword);
      } catch(e) { alert('Сервер недоступен'); }
    }
    async function setUserPassword(login) {
      const adminKey = prompt('Ключ администратора (ADMIN_KEY):');
      if (!adminKey) return;
      const newPassword = prompt('Новый пароль для ' + login + ' (минимум 4 символа):');
      if (!newPassword) return;
      try {
        const r = await fetch('/admin/users/set-password', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey, login, newPassword }) });
        const d = await r.json();
        if (!d.ok) { alert('Ошибка: ' + (d.error||'')); return; }
        alert('Пароль для ' + login + ' изменён.');
      } catch(e) { alert('Сервер недоступен'); }
    }
    function setWebhook() {
