...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/auth/register') {
    try {
      ...
      const user = { login: loginNorm, salt, hash, incoming: [], verified: false, displayName: null };
      users.set(key, user);
      saveUsers();
  ...
  if (req.method === 'POST' && req.url === '/auth/login') {
    try {
      ...
      sessions.set(token, key);
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, token, login: user.login, verified: !!user.verified, displayName: user.displayName || null }));
    } catch(e) {
      res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:e.message}));
    }
    return;
  ...
  if (req.method === 'POST' && req.url === '/notes/share') {
    try {
      ...
      const sharedNote = { ...note, sharedBy: (fromUser?.verified && fromUser?.displayName) || fromUser?.login || fromKey, shared: true, id: note.id };
  ...
  if (req.method === 'GET' && req.url.startsWith('/users/search')) {
    ...
    const results = [...users.values()]
      ...
      .slice(0, 50)
      .map(u => ({ login: u.login, notesCount: (u.notes || []).length, updated: u.notesUpdated || 0, disabled: !!u.disabled, verified: !!u.verified, displayName: u.displayName || null }));
    res.writeHead(200, { 'Content-Type': 'application/json' });
...
<body>
  ...
  <div class="info-card" style="max-width:820px;width:100%;">
    ...
    <div style="font-size:10px;color:var(--text3);margin-top:8px;font-weight:600;">Показаны логин, число заметок и дата последней синхронизации, официальный статус (галочка) и второй никнейм. Пароли и хэши паролей здесь никогда не показываются. «Сбросить пароль» выдаёт случайный новый пароль, «Задать пароль» — конкретный, «📨 Код в Telegram» — присылает пользователю код смены пароля прямо в Telegram, «✓ Официальный» — выдаёт/снимает галочку официального аккаунта. Все действия требуют ключ администратора (ADMIN_KEY).</div>
  </div>
  ...
  <script>
    ...
    function renderUsers(list) {
      ...
      list.forEach(u => {
        ...
        nameLine.textContent = (u.verified ? '✓ ' : '') + u.login + (u.displayName ? ' («' + u.displayName + '»)' : '') + (u.disabled ? ' (отключён)' : '');
    ...
    async function resetUserPassword(login) {
      const adminKey = await getAdminKey('Сбросить пароль');
      if (!adminKey) return;
