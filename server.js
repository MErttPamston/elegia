...
// Также: помечает флагом это же на копии заметки у ВЛАДЕЛЬЦА (recipientDone),
// чтобы это было видно прямо на карточке заметки при следующей синхронизации,
// и кладёт запись во внутрисайтовую ленту уведомлений владельца.
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/auth/register') {
    try {
      ...
      const user = { login: loginNorm, salt, hash, incoming: [], notifications: [], equipment: [], equipmentIncoming: [], verified: false, displayName: null, plan: 'free' };
      users.set(key, user);
      saveUsers();
  ...
  // ── POST /equipment/share {token, targetLogin, scope, scopeLabel, orgs} ────
  // Раздельный шаринг: либо всё дерево оборудования, либо одна организация,
  // либо одно место — приходит получателю ОТДЕЛЬНО (в свой список "Полученное"),
  // не смешиваясь с его собственным деревом.
  if (req.method === 'POST' && req.url === '/equipment/share') {
    try {
      const { token, targetLogin, scope, scopeLabel, orgs } = JSON.parse(await readBody(req));
      const fromKey = sessions.get(token);
      if (!fromKey) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
      const targetKey = String(targetLogin||'').trim().toLowerCase();
      const target = users.get(targetKey);
      ...
      pushNotification(target, {
        ...
        text: `${fromUser?.login || fromKey} поделился(-ась) с вами: ${pkg.scopeLabel}`,
  ...
  if (req.method === 'GET' && req.url.startsWith('/equipment/inbox')) {
    ...
    if (user) { user.equipmentIncoming = []; saveUsers(); }
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, packages: incoming }));
    return;
  }
