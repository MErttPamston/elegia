...
// Страховка: даже если по какой-то причине отложенный бэкап после saveUsers()
// не успеет сработать (например, сервер перезапустился в первые секунды),
// раз в 5 минут всё равно сохраняем текущее состояние в Gist.
setInterval(backupUsersToGist, 5 * 60 * 1000);
...
function isRegistrationKeyValid(key) {
  ...
  // Временный код с страницы статуса сервера — регистр не важен (буквы/цифры)
  if (isRegCodeActive() && String(key).toUpperCase() === regCode.code) return true;
  return false;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/admin/regcode/create') {
    try {
      ...
      const REG_CODE_CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
      let code = '';
      const randBytes = crypto.randomBytes(4);
      for (let i = 0; i < 4; i++) code += REG_CODE_CHARS[randBytes[i] % REG_CODE_CHARS.length];
      const now = Date.now();
  ...
  if (req.method === 'GET' && req.url === '/admin/backup/status') {
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, configured: !!(GIST_BACKUP_TOKEN && GIST_BACKUP_ID), userCount: users.size }));
    return;
  }
  if (req.method === 'POST' && req.url === '/admin/backup/now') {
    try {
      ...
      backupUsersToGist();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, userCount: users.size }));
    } catch(e) {
      res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:e.message}));
    }
    return;
  ...
  if (req.method === 'GET' && req.url === '/') {
    // ── Вход на страницу статуса только по ADMIN_KEY (HTTP Basic Auth) ──────
    // Браузер сам покажет системное окно логина. Логин можно ввести любой,
    // важен только пароль — он и есть ADMIN_KEY. Та же защита от перебора,
    // что и у остальных админ-функций (блокировка на 15 мин после 5 ошибок).
    const clientIp = getClientIp(req);
    const blockedUntil = ipBlockedUntil(clientIp);
    if (blockedUntil) {
      const waitMin = Math.max(1, Math.ceil((blockedUntil - Date.now()) / 60000));
      res.writeHead(429, { 'Content-Type': 'text/plain; charset=utf-8', 'WWW-Authenticate': 'Basic realm="Nota Admin"' });
      res.end(`Слишком много неверных попыток. Попробуйте через ${waitMin} мин.`);
      return;
    }
    let providedKey = '';
    ...
    if (authHeader.startsWith('Basic ')) {
      try {
        ...
        providedKey = decoded.slice(decoded.indexOf(':') + 1);
      } catch (e) {}
    }
    if (!process.env.ADMIN_KEY || providedKey !== process.env.ADMIN_KEY) {
      recordFailedAdminAttempt(clientIp);
      res.writeHead(401, { 'Content-Type': 'text/plain; charset=utf-8', 'WWW-Authenticate': 'Basic realm="Nota Admin"' });
      res.end('Нужен код доступа');
      return;
    }
...
<body>
  ...
  <div class="info-card" style="max-width:820px;width:100%;">
    ...
    <div style="font-size:10px;color:var(--text3);margin-top:8px;font-weight:600;">Это то, что гарантирует: старые аккаунты не пропадают при обновлении сервера на Render (там диск стирается при каждом деплое), и в любой старый аккаунт всегда можно зайти. Бэкап автоматически сохраняется в приватный GitHub Gist при каждой регистрации/смене пароля. «Восстановить из бэкапа» — безопасная операция: она только добавляет аккаунты, которых сейчас нет на сервере, и никогда не трогает и не удаляет уже существующие. Требует переменные окружения GIST_BACKUP_TOKEN и GIST_BACKUP_ID на сервере — без них кнопки работать не будут.</div>
  </div>
  <div class="info-card" style="max-width:820px;width:100%;">
  ...
  <script>
    ...
    async function refreshBackupStatus() {
      ...
      try {
        ...
        el.innerHTML = d.configured
          ? '<span style="color:var(--accent2);">✅ Настроено</span> · аккаунтов на сервере: ' + d.userCount
          : '<span style="color:var(--red);">⚠️ Не настроено</span> — задайте GIST_BACKUP_TOKEN и GIST_BACKUP_ID в переменных окружения на Render, иначе аккаунты не переживут обновление сервера';
      } catch(e) { el.innerHTML = '<span style="color:var(--red);">Сервер не отвечает</span>'; }
    }
    async function backupNow() {
      const adminKey = await adminPrompt('Ключ администратора (ADMIN_KEY)', { password:true, title:'Сделать бэкап сейчас' });
      if (!adminKey) return;
      try {
    ...
    async function restoreBackup() {
      const adminKey = await adminPrompt('Ключ администратора (ADMIN_KEY)', { password:true, title:'Восстановить из бэкапа' });
      if (!adminKey) return;
    ...
    function renderUsers(list) {
      ...
      list.forEach(u => {
        ...
        nameLine.textContent = u.login + (u.disabled ? ' (отключён)' : '');
        ...
        actions.appendChild(mkBtn('🔑 Сбросить пароль', () => resetUserPassword(u.login)));
