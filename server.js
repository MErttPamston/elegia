      ...
      if (warnBefore > 0) {
        ...
        for (let i = 0; i < times; i++) {
          ...
          if (warnT <= now && warnT > now - 24*3600*1000 && !fired[warnKey]) {
            ...
            let msg = `⏰ <b>Напоминание за ${leadLabel} — Элегия</b>\n\n📌 <b>${title}</b>`;
            try {
...
async function handleTelegramUpdate(update) {
  ...
  if (text === '/telegram') {
    await sendTelegramRaw(chatId, token, `🆔 <b>Ваш Chat ID:</b>\n<code>${chatId}</code>\n\nВставьте его в приложении Тихая → вкладка Telegram, чтобы привязать бота к этому сайту.`);
    return;
  }
  ...
  store.set(chatId, data);
  const sent = await sendTelegramRaw(chatId, token, `📝 Заметка добавлена: «${title}»\n\nОна появится в приложении Тихая при следующем открытии/синхронизации.\n✏️ Ответьте на это сообщение, чтобы изменить текст заметки.`);
...
<body>
  ...
  <div class="info-card">
    ...
    <span class="ep">GET /pending</span> — новые заметки/изменения из Telegram<br>
    <span class="ep">GET /tg/getMe</span> · <span class="ep">POST /tg/sendMessage</span> — прокси к Telegram API<br>
