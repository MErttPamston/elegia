...
async function handleWizardText(chatId, token, data, text) {
  ...
  if (w.step === 'title') {
    ...
    if (!d) { await sendTelegramRaw(chatId, token, '⚠️ Не понял дату. Формат: ДД.ММ.ГГГГ ЧЧ:ММ (например 25.12.2026 18:00), или нажмите «Оставить бездатной».'); return; }
