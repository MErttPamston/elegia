...
function checkAdminAccess(req, res, { password, code } = {}) {
  ...
  if (code && ADMIN_QUICK_CODES[String(code).trim()]) {
    resetAdminAttempts(ip);
    return true;
  }
  if (password && process.env.ADMIN_KEY && password === process.env.ADMIN_KEY) {
    resetAdminAttempts(ip);
    return true;
  }
  recordFailedAdminAttempt(ip);
  res.writeHead(403, {'Content-Type':'application/json'});
  res.end(JSON.stringify({ ok:false, error:'Неверный код или пароль' }));
  return false;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/admin/gate-check') {
    try {
      ...
      if (!checkAdminAccess(req, res, { password, code })) return;
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true }));
    } catch(e) {
      res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:e.message}));
