...
// ── Хранилище приложения "Учёт занятий" (index1.html, вкладка "Отчёт") ──────
// Раньше все данные (ученики/группы/оплата/коды) жили только в localStorage
// браузера. Теперь при каждом сохранении на клиенте вся база целиком
// отправляется сюда и пишется на диск рядом с server.js — так данные не
// теряются при очистке браузера и видны с любого устройства, у которого
// есть код доступа. Отдельный файл в том же приватном Gist используется для
// резервного копирования (по аналогии с бэкапом аккаунтов Nota выше).
const UCHET_FILE = require('path').join(__dirname, 'uchet.json');
const UCHET_GIST_FILENAME = 'nota-uchet-backup.json';
let uchetDB = null; // { codes, groups, students, days, payments } | null (ещё не синхронизировано)
let uchetGistBackupTimer = null;
let lastUchetBackupAt = 0;
let lastUchetBackupError = null;
function loadUchetDB() {
  try { uchetDB = JSON.parse(fs.readFileSync(UCHET_FILE, 'utf8')); }
  catch (e) { uchetDB = null; }
}
function saveUchetDB() {
  try { fs.writeFileSync(UCHET_FILE, JSON.stringify(uchetDB)); }
  catch (e) { console.error('[uchet] save error:', e.message); }
  clearTimeout(uchetGistBackupTimer);
  uchetGistBackupTimer = setTimeout(backupUchetToGist, 1500);
}
function backupUchetToGist() {
  return new Promise(resolve => {
    if (!GIST_BACKUP_TOKEN || !GIST_BACKUP_ID) { resolve({ ok:false, error:'not configured' }); return; }
    if (!uchetDB) { resolve({ ok:false, error:'nothing to back up yet' }); return; }
    const body = JSON.stringify({ files: { [UCHET_GIST_FILENAME]: { content: JSON.stringify(uchetDB) } } });
    const req = https.request(`https://api.github.com/gists/${GIST_BACKUP_ID}`, {
      method: 'PATCH',
      headers: {
        'Authorization': `token ${GIST_BACKUP_TOKEN}`,
      ...
      r.on('end', () => {
        if (r.statusCode >= 200 && r.statusCode < 300) {
          lastUchetBackupAt = Date.now(); lastUchetBackupError = null;
          resolve({ ok:true });
        } else {
          const msg = `GitHub ответил ${r.statusCode}: ${data.slice(0, 300)}`;
          lastUchetBackupError = msg; console.error('[uchet-gist] backup failed:', msg);
          resolve({ ok:false, error: msg });
        }
      });
    });
    req.on('error', e => { lastUchetBackupError = e.message; console.error('[uchet-gist] backup failed:', e.message); resolve({ ok:false, error: e.message }); });
    req.write(body); req.end();
  });
}
function restoreUchetFromGistIfEmpty() {
  if (uchetDB || !GIST_BACKUP_TOKEN || !GIST_BACKUP_ID) return;
  https.get(`https://api.github.com/gists/${GIST_BACKUP_ID}`, {
    headers: { 'Authorization': `token ${GIST_BACKUP_TOKEN}`, 'User-Agent': 'nota-server' }
  }, r => {
    let data = '';
    ...
    r.on('end', () => {
      try {
        const d = JSON.parse(data);
        const file = d.files && d.files[UCHET_GIST_FILENAME];
        if (file && file.content) {
          uchetDB = JSON.parse(file.content);
          saveUchetDB();
          console.log('[uchet-gist] Восстановлена база "Учёт занятий" из резервной копии');
        }
      } catch (e) { console.error('[uchet-gist] restore parse failed:', e.message); }
    });
  }).on('error', e => console.error('[uchet-gist] restore failed:', e.message));
}
loadUchetDB();
...
// Смена пароля больше НЕ доступна через код/Telegram/самообслуживание —
// единственный способ изменить пароль пользователя теперь /admin/users/set-password
// и /admin/users/reset-password (оба требуют ADMIN_KEY и доступны только из админ-панели).
...
// Коды быстрого входа в панель мониторинга — заменяют логин+пароль ADMIN_KEY.
// 6423 -> admin, 7833 -> admin1. Действуют наравне с ADMIN_KEY, под той же
// защитой от перебора (5 неверных попыток -> блокировка IP на 15 минут).
const ADMIN_QUICK_CODES = { '6423': 'admin', '7833': 'admin1' };
function checkAdminKey(req, res, providedKey) {
  ...
  const isQuickCode = providedKey && ADMIN_QUICK_CODES[String(providedKey).trim()];
  const isPasswordOk = providedKey && process.env.ADMIN_KEY && providedKey === process.env.ADMIN_KEY;
  if (!isQuickCode && !isPasswordOk) {
...
// ── Внутрисайтовые уведомления (лента действий) ─────────────────────────────
// Хранятся прямо в объекте пользователя (user.notifications), максимум 100
// последних, отдаются через /account/notifications. Каждое: { id, type,
// text, noteId, noteTitle, byLogin, at, read }.
const NOTIF_LIMIT = 100;
function pushNotification(user, notif) {
  if (!user) return;
  user.notifications = user.notifications || [];
  user.notifications.unshift({ id: crypto.randomBytes(8).toString('hex'), at: Date.now(), read: false, ...notif });
  if (user.notifications.length > NOTIF_LIMIT) user.notifications.length = NOTIF_LIMIT;
}

// ── Уведомление владельца о действиях получателя над общей заметкой ─────────
// Срабатывает и на «отметил(а) выполненной», и на «отредактировал(а)».
// Также: помечает флагом «выполнено получателем» копию заметки у ВЛАДЕЛЬЦА,
// чтобы это было видно прямо на карточке при следующей синхронизации, и кладёт
// запись во внутрисайтовую ленту уведомлений владельца.
function notifyOwnersOfSharedNoteActivity(user, incomingNotes) {
  try {
    const oldById = new Map((user.notes || []).map(n => [n.id, n]));
    for (const n of incomingNotes) {
      if (!n || !n.shared) continue;
      // ownerLogin — настоящий логин (для старых заметок, расшаренных до этого
      // обновления, его может не быть — тогда используем sharedBy как раньше,
      // это может быть отображаемым именем, а не логином, но это лучше, чем ничего).
      const ownerLoginKey = String(n.ownerLogin || n.sharedBy || '').toLowerCase();
      if (!ownerLoginKey) continue;
      const owner = users.get(ownerLoginKey);
      if (!owner) continue;
      const old = oldById.get(n.id);

      if (n.done && !n.doneNotified && (!old || !old.done)) {
        ...
        const ownerNote = (owner.notes || []).find(x => x.id === n.id);
        if (ownerNote) {
          ownerNote.recipientDone = true;
          ownerNote.recipientDoneBy = user.login;
          ownerNote.recipientDoneAt = Date.now();
          ownerNote.updated = Date.now();
        }
        pushNotification(owner, {
          type: 'note_done',
          text: `${user.login} отметил(а) вашу заметку «${n.title || '(без названия)'}» выполненной`,
          noteId: n.id, noteTitle: n.title || '', byLogin: user.login
        });
        n.doneNotified = true;
      }

      // Изменение содержимого (заголовок/текст) получателем — отдельное уведомление.
      if (old) {
        const contentChanged = (old.title||'') !== (n.title||'') || (old.plainText||'') !== (n.plainText||'');
        if (contentChanged) {
          pushNotification(owner, {
            type: 'note_edited',
            text: `${user.login} отредактировал(а) вашу общую заметку «${n.title || old.title || '(без названия)'}»`,
            noteId: n.id, noteTitle: n.title || '', byLogin: user.login
          });
        }
      }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/auth/register') {
    try {
      ...
      const user = { login: loginNorm, salt, hash, incoming: [], notifications: [], equipment: [], equipmentIncoming: [], noteShares: [], equipmentShares: [], pendingRevocations: [], verified: false, displayName: null, plan: 'free' };
      users.set(key, user);
      saveUsers();
  ...
  if (req.method === 'POST' && req.url === '/account/notes') {
    try {
      ...
      notifyOwnersOfSharedNoteActivity(user, incomingNotes);
  ...
  if (req.method === 'POST' && req.url === '/notes/share') {
    try {
      ...
      // Запись о шаринге хранится у ВЛАДЕЛЬЦА — это и есть основа для отзыва
      // доступа (POST /notes/revoke-share) и для уведомлений о редактировании.
      fromUser.noteShares = fromUser.noteShares || [];
      const shareRec = { id: crypto.randomBytes(8).toString('hex'), noteId: note.id, targetLogin: targetKey, createdAt: Date.now(), revoked: false };
      fromUser.noteShares.push(shareRec);
      const sharedNote = { ...note, sharedBy: (fromUser?.verified && fromUser?.displayName) || fromUser?.login || fromKey, ownerLogin: fromUser?.login || fromKey, sharedByVerified: !!fromUser?.verified, sharedByPlan: fromUser?.plan || 'free', shared: true, id: note.id, shareId: shareRec.id };
      target.incoming = target.incoming || [];
      target.incoming.push(sharedNote);
      pushNotification(target, {
        type: 'note_shared',
        text: `${fromUser?.login || fromKey} поделился(-ась) с вами заметкой «${note.title || '(без названия)'}»`,
        noteId: note.id, noteTitle: note.title || '', byLogin: fromUser?.login || fromKey
      });
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, shareId: shareRec.id }));
    } catch(e) {
      res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:e.message}));
    }
    return;
  ...
  // ── GET /notes/inbox?token=XXX — забрать заметки, которыми поделились, и
  // список id заметок, доступ к которым владелец отозвал (их надо убрать
  // локально у получателя). ───────────────────────────────────────────────
  if (req.method === 'GET' && req.url.startsWith('/notes/inbox')) {
    ...
    const pending = user?.pendingRevocations || [];
    const revokedNoteIds = pending.filter(r=>r.type==='note').map(r=>r.noteId);
    if (user) {
      user.incoming = [];
      user.pendingRevocations = pending.filter(r=>r.type!=='note');
      saveUsers();
    }
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, notes: incoming, revokedNoteIds }));
    return;
  }

  // ── POST /notes/revoke-share {token, shareId} — владелец забирает доступ
  // к расшаренной заметке обратно. Заметка будет удалена у получателя при
  // следующем опросе /notes/inbox. ───────────────────────────────────────
  if (req.method === 'POST' && req.url === '/notes/revoke-share') {
    try {
      const { token, shareId } = JSON.parse(await readBody(req));
      const fromKey = sessions.get(token);
      if (!fromKey) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
      const fromUser = users.get(fromKey);
      const rec = (fromUser?.noteShares||[]).find(s=>s.id===shareId);
      if (!rec) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Шаринг не найден'})); return; }
      rec.revoked = true;
      const target = users.get(rec.targetLogin);
      if (target) {
        target.pendingRevocations = target.pendingRevocations || [];
        target.pendingRevocations.push({ type:'note', noteId: rec.noteId, shareId, at: Date.now() });
      }
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true }));
  ...
  // ── POST /account/equipment {token, equipment} — сохранить дерево
  // Организация→Место→Оборудование целиком (полностью отдельная структура
  // от заметок; last-write-wins так же, как и заметки). ────────────────────
  if (req.method === 'POST' && req.url === '/account/equipment') {
    try {
      const { token, equipment } = JSON.parse(await readBody(req));
      const key = sessions.get(token);
      if (!key) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
      const user = users.get(key);
      if (!user) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Аккаунт не найден'})); return; }
      ...
      user.equipmentUpdated = Date.now();
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, updated: user.equipmentUpdated }));
    } catch(e) {
      res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:e.message}));
    }
    return;
  }
  // ── GET /account/equipment?token=XXX — забрать дерево оборудования ─────────
  if (req.method === 'GET' && req.url.startsWith('/account/equipment')) {
    const urlObj = new URL(req.url, 'http://localhost');
    const token = urlObj.searchParams.get('token');
    const key = sessions.get(token);
    if (!key) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
    const user = users.get(key);
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, equipment: user?.equipment || [], updated: user?.equipmentUpdated || 0 }));
    return;
  }
  // ── POST /equipment/share {token, targetLogin, scope, scopeLabel, orgId,
  // placeId, permission, orgs} — раздельный шаринг: всё дерево / организация /
  // место. Приходит получателю ОТДЕЛЬНО (в "Полученное"), не подмешиваясь в
  // его собственное дерево. permission: 'view' (только смотреть) или 'edit'
  // (получатель может менять статус — изменения уйдут обратно владельцу). ──
  if (req.method === 'POST' && req.url === '/equipment/share') {
    try {
      const { token, targetLogin, scope, scopeLabel, orgId, placeId, permission, orgs } = JSON.parse(await readBody(req));
      const fromKey = sessions.get(token);
      if (!fromKey) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
      const targetKey = String(targetLogin||'').trim().toLowerCase();
      const target = users.get(targetKey);
      if (!target) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Пользователь с таким логином не найден'})); return; }
      if (!Array.isArray(orgs) || !orgs.length) { res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Нечего отправлять'})); return; }
      const fromUser = users.get(fromKey);
      const perm = permission === 'edit' ? 'edit' : 'view';
      fromUser.equipmentShares = fromUser.equipmentShares || [];
      const shareRec = {
        id: crypto.randomBytes(8).toString('hex'),
        targetLogin: targetKey,
        scope: String(scope||'all'),
        scopeLabel: String(scopeLabel||'оборудование'),
        orgId: orgId || null,
      ...
      fromUser.equipmentShares.push(shareRec);
      const pkg = {
        id: crypto.randomBytes(8).toString('hex'),
        shareId: shareRec.id,
        ownerLogin: fromUser?.login || fromKey,
        scope: shareRec.scope,
        scopeLabel: shareRec.scopeLabel,
        permission: perm,
        fromLogin: fromUser?.login || fromKey,
        at: Date.now(),
        orgs
      };
      target.equipmentIncoming = target.equipmentIncoming || [];
      target.equipmentIncoming.push(pkg);
      pushNotification(target, {
        type: 'equipment_shared',
        text: `${fromUser?.login || fromKey} поделился(-ась) с вами: ${pkg.scopeLabel}${perm==='edit' ? ' (можно менять статус)' : ''}`,
        byLogin: fromUser?.login || fromKey
      });
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, shareId: shareRec.id }));
  ...
  // ── GET /equipment/inbox?token=XXX — забрать полученные пакеты оборудования,
  // и id пакетов (shareId), доступ к которым владелец отозвал ────────────────
  if (req.method === 'GET' && req.url.startsWith('/equipment/inbox')) {
    const urlObj = new URL(req.url, 'http://localhost');
    const token = urlObj.searchParams.get('token');
    const key = sessions.get(token);
    if (!key) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
    const user = users.get(key);
    const incoming = user?.equipmentIncoming || [];
    const pending = user?.pendingRevocations || [];
    const revokedShareIds = pending.filter(r=>r.type==='equipment').map(r=>r.shareId);
    if (user) {
      user.equipmentIncoming = [];
      user.pendingRevocations = pending.filter(r=>r.type!=='equipment');
      saveUsers();
    }
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, packages: incoming, revokedShareIds }));
    return;
  }
  // ── GET /equipment/shares?token=XXX — список СОБСТВЕННЫХ активных шарингов
  // владельца (для экрана "Отправленное" с возможностью отозвать доступ) ─────
  if (req.method === 'GET' && req.url.startsWith('/equipment/shares')) {
    const urlObj = new URL(req.url, 'http://localhost');
    const token = urlObj.searchParams.get('token');
    const key = sessions.get(token);
    if (!key) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
    const user = users.get(key);
    const shares = (user?.equipmentShares || []).filter(s=>!s.revoked);
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, shares }));
    return;
  }
  // ── POST /equipment/revoke-share {token, shareId} — владелец отзывает доступ ─
  if (req.method === 'POST' && req.url === '/equipment/revoke-share') {
    try {
      const { token, shareId } = JSON.parse(await readBody(req));
      const fromKey = sessions.get(token);
      if (!fromKey) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
      const fromUser = users.get(fromKey);
      const rec = (fromUser?.equipmentShares||[]).find(s=>s.id===shareId);
      if (!rec) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Шаринг не найден'})); return; }
      rec.revoked = true;
      const target = users.get(rec.targetLogin);
      if (target) {
        target.pendingRevocations = target.pendingRevocations || [];
        target.pendingRevocations.push({ type:'equipment', shareId, at: Date.now() });
      }
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true }));
  ...
  // ── POST /equipment/update-status {token, ownerLogin, shareId, orgId,
  // placeId, itemId, status} — получатель с правом "может менять статус"
  // меняет статус конкретной единицы оборудования ПРЯМО В ДЕРЕВЕ ВЛАДЕЛЬЦА.
  // Записывается в историю изменений (кто/когда), владелец получает
  // уведомление. ──────────────────────────────────────────────────────────
  if (req.method === 'POST' && req.url === '/equipment/update-status') {
    try {
      const { token, ownerLogin, shareId, orgId, placeId, itemId, status } = JSON.parse(await readBody(req));
      const recipientKey = sessions.get(token);
      if (!recipientKey) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
      const recipient = users.get(recipientKey);
      const owner = users.get(String(ownerLogin||'').toLowerCase());
      if (!owner) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Владелец не найден'})); return; }
      const rec = (owner.equipmentShares||[]).find(s=>s.id===shareId);
      if (!rec || rec.revoked) { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Доступ отозван или не найден'})); return; }
      if (rec.targetLogin !== recipientKey) { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Это не ваш доступ'})); return; }
      if (rec.permission !== 'edit') { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'У вас только просмотр, менять статус нельзя'})); return; }
      // Проверяем, что запрошенная единица действительно попадает в границы
      // выданного доступа (scope: всё / конкретная организация / конкретное место).
      if (rec.scope === 'org' && rec.orgId !== orgId) { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Вне области доступа'})); return; }
      if (rec.scope === 'place' && (rec.orgId !== orgId || rec.placeId !== placeId)) { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Вне области доступа'})); return; }
      const validStatuses = ['поставлен','забран','в ремонте','удалён'];
      if (!validStatuses.includes(status)) { res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Некорректный статус'})); return; }
      const org = (owner.equipment||[]).find(o=>o.id===orgId);
      const place = org?.places.find(p=>p.id===placeId);
      const item = place?.items.find(i=>i.id===itemId);
      if (!item) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Оборудование не найдено (возможно, уже удалено владельцем)'})); return; }
      const prevStatus = item.status;
      item.status = status;
      item.updated = Date.now();
      item.history = item.history || [];
      item.history.unshift({ status, prevStatus, by: recipient?.login || recipientKey, at: Date.now() });
      place.updated = Date.now();
      owner.equipmentUpdated = Date.now();
      pushNotification(owner, {
        type: 'equipment_edited',
        text: `${recipient?.login || recipientKey} изменил(а) статус «${item.name||'оборудования'}»: ${prevStatus||'—'} → ${status}`,
        byLogin: recipient?.login || recipientKey
      });
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true }));
  ...
  // ── GET /account/notifications?token=XXX — лента внутрисайтовых уведомлений ─
  // (кто-то поделился заметкой, получатель отметил её выполненной, и т.п.)
  if (req.method === 'GET' && req.url.startsWith('/account/notifications')) {
    const urlObj = new URL(req.url, 'http://localhost');
    const token = urlObj.searchParams.get('token');
    const key = sessions.get(token);
    if (!key) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
    ...
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, notifications: list, unread: list.filter(n=>!n.read).length }));
    return;
  }
  // ── GET /uchet/data?code=XXXX — забрать всю базу "Учёт занятий" целиком.
  // Код должен существовать в самой базе (codes) — так же, как раньше
  // проверялось только на клиенте. Если базы на сервере ещё нет (первый
  // запуск), отдаём db:null — клиент поймёт, что нужно залить свою локальную
  // копию первой (см. POST ниже).
  if (req.method === 'GET' && req.url.startsWith('/uchet/data')) {
    const urlObj = new URL(req.url, 'http://localhost');
    const code = String(urlObj.searchParams.get('code') || '').trim();
    if (!uchetDB) { res.writeHead(200,{'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:true, db:null })); return; }
    const entry = (uchetDB.codes || []).find(c => c.code === code);
    if (!entry) { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:false, error:'Неверный код доступа' })); return; }
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, db: uchetDB }));
    return;
  }
  // ── POST /uchet/data {code, db} — сохранить всю базу целиком (вызывается
  // после каждого локального saveDB() в index1.html) ─────────────────────────
  if (req.method === 'POST' && req.url === '/uchet/data') {
    try {
      const { code, db } = JSON.parse(await readBody(req));
      const trimmedCode = String(code || '').trim();
      // Разрешаем самый первый заливаемый снимок без проверки кода (сервер ещё
      // пуст — до этого момента доступ и так был только у того, кто открыл
      // приложение с правильным кодом на клиенте), дальше — код обязателен и
      // должен присутствовать в уже сохранённой базе.
      if (uchetDB) {
        const entry = (uchetDB.codes || []).find(c => c.code === trimmedCode);
        if (!entry) { res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:false, error:'Неверный код доступа' })); return; }
      }
      if (!db || typeof db !== 'object') { res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:false, error:'Пустые данные' })); return; }
      uchetDB = db;
      saveUchetDB();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, updated: Date.now() }));
    } catch (e) { res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ ok:false, error:'Ошибка запроса' })); }
    return;
  }
  // ── GET /admin/uchet/backup/status — статус бэкапа базы "Учёт занятий" ────
  if (req.method === 'GET' && req.url === '/admin/uchet/backup/status') {
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, configured: !!(GIST_BACKUP_TOKEN && GIST_BACKUP_ID), lastBackupAt: lastUchetBackupAt, lastBackupError: lastUchetBackupError, hasData: !!uchetDB }));
    return;
  }
  // ── POST /admin/uchet/backup/now {adminKey} — сделать бэкап "Отчёта" вручную
  if (req.method === 'POST' && req.url === '/admin/uchet/backup/now') {
    try {
      ...
      if (!uchetDB) { res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'На сервере пока нет данных "Учёта занятий" — откройте вкладку «Отчёт» хотя бы раз, чтобы данные синхронизировались'})); return; }
      const r = await backupUchetToGist();
  ...
  // ── GET /account/uchet-access?token=XXX — проверить, выдан ли этому аккаунту
  // администратором доступ к вкладке "Отчёт" (index1.html), без ввода кода ──
  if (req.method === 'GET' && req.url.startsWith('/account/uchet-access')) {
    const urlObj = new URL(req.url, 'http://localhost');
    const token = urlObj.searchParams.get('token');
    const key = sessions.get(token);
    if (!key) { res.writeHead(401,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Не авторизован'})); return; }
    const user = users.get(key);
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({ ok:true, uchetAccess: !!(user && user.uchetAccess) }));
    return;
  }
  // ── POST /account/notifications/read {token, id?} — пометить одно (или все,
  // если id не передан) уведомление прочитанным ────────────────────────────
  if (req.method === 'POST' && req.url === '/account/notifications/read') {
    try {
      ...
      if (user) {
        user.notifications = user.notifications || [];
        if (id) { const n = user.notifications.find(x=>x.id===id); if (n) n.read = true; }
        else user.notifications.forEach(n => n.read = true);
        saveUsers();
      }
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true }));
  ...
  // ── POST /admin/users/toggle-uchet-access — выдать/снять доступ к вкладке
  // "Отчёт" (встроенное приложение "Учёт занятий", index1.html) ──────────────
  if (req.method === 'POST' && req.url === '/admin/users/toggle-uchet-access') {
    try {
      const { adminKey, login, access } = JSON.parse(await readBody(req));
      if (!checkAdminKey(req, res, adminKey)) return;
      const user = users.get(String(login||'').trim().toLowerCase());
      if (!user) { res.writeHead(404,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Пользователь не найден'})); return; }
      user.uchetAccess = !!access;
      saveUsers();
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true, uchetAccess: user.uchetAccess }));
    } catch(e) { res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Ошибка запроса'})); }
    return;
  }
  ...
  if (req.method === 'GET' && req.url.startsWith('/users/search')) {
    ...
    const results = [...users.values()]
      ...
      .slice(0, 50)
      .map(u => ({ login: u.login, notesCount: (u.notes || []).length, updated: u.notesUpdated || 0, disabled: !!u.disabled, verified: !!u.verified, displayName: u.displayName || null, plan: u.plan || 'free', uchetAccess: !!u.uchetAccess }));
    res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.method === 'POST' && req.url === '/admin/gate-check') {
    try {
      const { password, code } = JSON.parse(await readBody(req));
      if (!checkAdminKey(req, res, password || code)) return;
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({ ok:true }));
    } catch(e) {
      res.writeHead(400,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:e.message}));
...
<body>
  ...
  <div class="info-card" style="max-width:820px;width:100%;">
    <b>📊 Резервное копирование «Отчёта» (Учёт занятий):</b><br>
    <div id="uchet-backup-status" style="margin-top:8px;font-size:13px;font-weight:700;color:var(--text2);">Загрузка…</div>
    <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
      <button class="ep" style="cursor:pointer;border:none;" onclick="uchetBackupNow()">💾 Сделать бэкап Отчёта</button>
    </div>
    <div style="font-size:10px;color:var(--text3);margin-top:8px;font-weight:600;">Отдельный бэкап данных приложения «Учёт занятий» (ученики, группы, оплата, коды), которое открывается через вкладку «Отчёт» в Nota. Сохраняется в тот же приватный Gist, отдельным файлом от аккаунтов Nota.</div>
  </div>
  <div class="info-card" style="max-width:820px;width:100%;">
  ...
  <script>
    ...
    async function gateSubmitLogin(e) {
      e.preventDefault();
      const login = document.getElementById('gate-login-input').value.trim();
      const errEl = document.getElementById('gate-error');
      errEl.textContent = '';
      if (login === '6423' || login === '7833') {
        try {
          const r = await fetch('/admin/gate-check', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code: login }) });
      ...
      if (loginLower === 'admin' || loginLower === 'admin1') {
        gateIsAdmin = true;
    ...
    async function refreshUchetBackupStatus() {
      const el = document.getElementById('uchet-backup-status');
      if (!el) return;
      try {
        const r = await fetchWithTimeout('/admin/uchet/backup/status');
        const d = await r.json();
        if (!d.configured) {
          el.innerHTML = '<span style="color:var(--red);">⚠️ Не настроено</span> — задайте GIST_BACKUP_TOKEN и GIST_BACKUP_ID в переменных окружения на сервере';
        } else {
          const esc = s => String(s).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
          const when = d.lastBackupAt ? new Date(d.lastBackupAt).toLocaleString('ru-RU') : 'ещё ни разу';
          let html = '<span style="color:var(--accent2);">✅ Настроено</span> · данные на сервере: ' + (d.hasData ? 'есть' : 'пока нет')
    ...
    async function uchetBackupNow() {
      const adminKey = await getAdminKey('Сделать бэкап Отчёта');
      if (!adminKey) return;
      try {
        const r = await fetchWithTimeout('/admin/uchet/backup/now', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey }) });
        const d = await r.json();
        if (!d.ok) { await adminAlert(d.error || 'Ошибка', 'Ошибка'); return; }
        await adminAlert('Данные «Отчёта» сохранены в резервную копию.', 'Готово');
        refreshUchetBackupStatus();
    ...
    function renderUsers(list) {
      ...
      list.forEach(u => {
        ...
        actions.appendChild(mkBtn(u.uchetAccess ? '📊 Забрать доступ к Отчёту' : '📊 Выдать доступ к Отчёту', () => toggleUserUchetAccess(u.login, !u.uchetAccess)));
    ...
    async function toggleUserUchetAccess(login, access) {
      const adminKey = await getAdminKey(access ? 'Выдать доступ к Отчёту' : 'Забрать доступ к Отчёту');
      if (!adminKey) return;
      try {
        const r = await fetchWithTimeout('/admin/users/toggle-uchet-access', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ adminKey, login, access }) });
        const d = await r.json();
        if (!d.ok) { await adminAlert(d.error || 'Ошибка', 'Ошибка'); return; }
        runUserSearch(searchInput.value.trim());
      } catch(e) { await adminAlert('Сервер недоступен', 'Ошибка'); }
