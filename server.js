...
async function handleTelegramUpdate(update) {
  ...
  if (text === '/notes' || text.startsWith('/notes ')) {
    ...
    if (!data || !Array.isArray(data.notes)) {
      await sendTelegramRaw(chatId, data?.account?.token, '📭 <b>Нет данных</b>\n\nОткройте приложение Элегия — оно автоматически синхронизирует заметки с сервером.', store.values().next().value?.account?.token);
      return;
    }
    const { notes, account } = data;
    const active = notes.filter(n => !n.done);
    if (!active.length) {
      await sendTelegramRaw(chatId, account.token, '✨ <b>Активных заметок нет</b>');
      return;
    }
    const now = Date.now();
    ...
    await sendTelegramRaw(chatId, account.token, msg2.trim());
  } else if (text === '/start') {
    await sendTelegramRaw(chatId, null, `👋 <b>Элегия — сервер уведомлений</b>\n\nДоступные команды:\n/notes — все активные заметки`, null, chatId);
  }
}
...
<body>
  ...
  <div class="info-card">
    ...
    <span class="ep">POST /telegram</span> — webhook для Telegram команд (/notes, /start)<br>
