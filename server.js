...
function checkAll() {
  ...
  for (const [chatid, data] of store.entries()) {
    ...
    // tzOffset — смещение часового пояса браузера в минутах (например -180 для UTC+3)
    // Браузер присылает его в поле account.tzOffset
    const tzOffset = (account.tzOffset !== undefined) ? account.tzOffset : 0;

    for (const n of notes) {
      if (!n.reminder || n.done) continue;
      // Если reminder без timezone (нет Z и нет +/-HH:MM) — добавляем смещение вручную
      ...
      if (hasTimezone) {
        ...
        // Трактуем как локальное время браузера, применяем его tzOffset
        // tzOffset в JS = -getTimezoneOffset(), т.е. для UTC+3 это +180
        // new Date(str) без tz = UTC, поэтому вычитаем смещение
