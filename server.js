...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/auth/register') {
    try {
      ...
      if (!process.env.REGISTRATION_KEY || regKey !== process.env.REGISTRATION_KEY) {
        res.writeHead(403,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:false,error:'Регистрация закрыта — нужен секретный код'})); return;
      }
