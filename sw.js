...
async function checkReminders() {
  ...
  for (const n of notes) {
    ...
    if (t <= now && !fired[key]) {
      fired[key] = true;
      changed = true;
      // Всплывающие уведомления браузера (showNotification) больше не
      // используются — напоминание уходит только в Telegram и во
      // внутрисайтовую ленту, пока сайт открыт.
      if (account) await sendTelegram(account, n);
    }

    // 1-hour-before — обрабатывается сервером (server.js), SW не отправляет
  }
